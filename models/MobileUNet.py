...
def ConvBlock(inputs, n_filters, kernel_size=[3, 3], training=False):
    """
    ...
    net = tf.layers.batch_normalization(net, training=training, fused=fused)
    # net = tf.keras.layers.BatchNormalization(net, training=training)
    # net = tf.layers.batch_normalization(net, training=training)
    # net = slim.batch_norm(net, fused=True, training=training)
    net = tf.nn.relu(net)
    return net


def DepthwiseSeparableConvBlock(inputs, n_filters, kernel_size=[3, 3], training=False):
    """
...
def conv_transpose_block(inputs, n_filters, kernel_size=[3, 3], training=False):
    """
...
def build_mobile_unet(inputs, preset_model, num_classes, is_training):
    ...
    # Downsampling path #
    #####################
    net = ConvBlock(inputs, 64, training=is_training)


    net = DepthwiseSeparableConvBlock(net, 64, training=is_training)
    net = slim.pool(net, [2, 2], stride=[2, 2], pooling_type='MAX')
    skip_1 = net

    net = DepthwiseSeparableConvBlock(net, 128, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 128, training=is_training)
    net = slim.pool(net, [2, 2], stride=[2, 2], pooling_type='MAX')
    skip_2 = net

    net = DepthwiseSeparableConvBlock(net, 256, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 256, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 256, training=is_training)
    net = slim.pool(net, [2, 2], stride=[2, 2], pooling_type='MAX')
    skip_3 = net

    net = DepthwiseSeparableConvBlock(net, 512, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 512, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 512, training=is_training)
    net = slim.pool(net, [2, 2], stride=[2, 2], pooling_type='MAX')
    skip_4 = net
    ...
    # Upsampling path #
    #####################
    net = conv_transpose_block(net, 512, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 512, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 512, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 512, training=is_training)
    if has_skip:
    ...
    net = conv_transpose_block(net, 256, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 256, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 256, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 128, training=is_training)
    if has_skip:
    ...
    net = conv_transpose_block(net, 128, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 128, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 64, training=is_training)
    if has_skip:
        net = tf.add(net, skip_1)

    net = conv_transpose_block(net, 64, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 64, training=is_training)
    net = DepthwiseSeparableConvBlock(net, 64, training=is_training)
